...
public:
  ...
  TableParams load_or_init_meta(uint64_t n, int k, double load_factor, std::optional<uint64_t> seed1,
  ...
  std::vector<BinStats> bin_stats(const TableParams& p, uint64_t bin_start, uint64_t bin_count);

  // Snapshot for Canvas grid visualization
  // returns rows: [idx, fp, reserved]
  std::vector<std::array<uint64_t, 3>> snapshot_bins(const TableParams& p, uint64_t bin_start, uint64_t bin_count);

private:
  Db& db_;

  // depth -> count (0..k). We also keep one extra bucket for depth==k+1 (fallback marker).
  // We avoid std::vector<std::atomic<...>> because atomics are non-movable.
  ...
  std::optional<uint64_t> claim_empty_slot_in_range(const std::string& table, uint64_t start, uint64_t end, uint64_t* probes);
  bool insert_into_fallback(const std::string& table, const TableParams& p, uint64_t key, uint16_t fp, uint64_t b,
                            std::vector<KickStep>* trace, uint64_t* probes);
  void erase_from_fallback_if_present(const TableParams& p, uint64_t key, uint16_t fp, uint64_t b,
                                      std::vector<KickStep>* trace);

  bool insert_with_kick(const std::string& table, const TableParams& p, const HashFunc& hf, uint64_t key, uint16_t fp,
                        uint64_t depth, std::vector<KickStep>* trace, uint64_t* probes);

  // Insert implementation targeting a specific physical table name.
  InsertResult insert_key_into_table(const std::string& table, const TableParams& p, uint64_t key, bool with_trace);
};
