...
// Phase3: LocalQueryRouter（正确版，工程实现）
// - key -> (cubby, slot)
// - 用一个二叉 trie 管理哈希后的 bit 串（本实现用 64-bit hash 串），并序列化为 bitstream
// - 为了先跑通：insert/erase 后允许重建 trie/编码（元素数为 polylog，重建成本可接受）
class Router {
public:
...
private:
  ...
  static uint64_t key_bits(uint64_t key) {
    // 与设计文档一致：router 对键使用随机 bit 串；这里用 splitmix64 作为稳定 hash。
