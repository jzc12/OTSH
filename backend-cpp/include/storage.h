...
// 与设计文档 `metrics` 表一致的单条样本（批量 flush 到 MySQL）。
struct SqlMetricRow {
  int64_t snapshot_id = 0;
  std::string operation_type; // insert | query | delete
  int probe_count = 0;
  int kick_count = 0;
...
public:
  ...
  virtual StorageResult for_each_key(const std::function<void(uint64_t)> &cb) = 0;
  ...
  virtual int64_t analytics_create_snapshot(const std::string &snapshot_tag,
                                            uint64_t n, uint64_t N, uint64_t K) {
    (void)snapshot_tag;
    (void)n;
    (void)N;
    (void)K;
  ...
  virtual StorageResult analytics_replace_structure(
