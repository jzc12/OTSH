      ...
      return true; });
        ...
        static void sync_facility_bucket(Facility &f, size_t b)
        {
            if (b >= f.ma.size() || b >= f.D.size())
                return;
            const FacilityBucketMeta meta{
                .entry_count = static_cast<uint32_t>(
                    std::min<size_t>(f.D[b].entry_count(),
                                     static_cast<size_t>(0xffffffffULL))),
                .encoded_bits = static_cast<uint32_t>(
                    std::min<uint64_t>(f.D[b].bits_total(), 0xffffffffULL)),
                .fingerprint = f.D[b].fingerprint()};
            f.ma.update(b, encode_facility_bucket_meta(meta), 128);
        }
