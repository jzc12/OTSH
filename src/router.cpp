...
namespace otsh
{
    ...
    {
        uint64_t h = 0x9e3779b97f4a7c15ULL ^ static_cast<uint64_t>(entries_.size());
        for (const auto &e : entries_)
        {
            const uint64_t slot = static_cast<uint64_t>(e.v.second);
            const uint64_t ptr =
                static_cast<uint64_t>(reinterpret_cast<std::uintptr_t>(e.v.first));
            h ^= splitmix64(e.key ^ (slot << 1) ^ (ptr << 7));
            h = (h << 13) | (h >> 51);
        }
        return h;
    }
