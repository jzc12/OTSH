...
int main()
{
    ...
    for (const char *pid : {"k3_1e4", "k5_1e4", "k7"})
    {
...

    {
        ...
        const double lgK = std::log2(static_cast<double>(std::max<uint64_t>(2, d.K)));
        run_case("kkick_s0_is_K", kkick_bin_size(0, d.K, d.n_hint) == d.K, cases);
        const size_t expect_s1 =
            static_cast<size_t>(std::max(1.0, std::floor(std::pow(lgK, 6.0))));
        run_case("kkick_s1_log6K", kkick_bin_size(1, d.K, d.n_hint) == expect_s1,
                 cases);
...

    {
        ...
        const auto logical_second = ma.access_logical(1);
        run_case("miniarray_logical_access",
                 ma.occupied_count() == 4 &&
                     logical_second.has_value() && !logical_second->empty() &&
                     (*logical_second)[0] == 0x42ULL,
                 cases);
    }

    {
        ...
        for (uint64_t i = 0; i < 40; ++i)
            ma.insert_logical(static_cast<size_t>(i), MiniArray::Bits{i + 1}, 8);
        ma.insert_logical(10, MiniArray::Bits{0xabcULL}, 12);
        const auto at10 = ma.access_logical(10);
        const auto at11 = ma.access_logical(11);
        run_case("miniarray_dynamic_insert_split",
                 ma.occupied_count() == 41 && at10.has_value() &&
                     !at10->empty() && (*at10)[0] == 0xabcULL &&
                     at11.has_value() && !at11->empty() && (*at11)[0] == 11,
                 cases);
        ma.delete_logical(10);
        const auto after_delete = ma.access_logical(10);
        run_case("miniarray_dynamic_delete_merge",
                 ma.occupied_count() == 40 && after_delete.has_value() &&
                     !after_delete->empty() && (*after_delete)[0] == 11,
                 cases);
    }

    {
        KKickGeometry geom(3, 64, 64, 10'000);
        BinFreeMap free_map;
        free_map.reset(&geom, 64);
        for (size_t i = 0; i < 64; ++i)
            free_map.mark_used(i);
        free_map.mark_free(37);
        free_map.mark_free(50);
        run_case("bin_free_map_range_select",
                 free_map.first_free_in_bin(BinRange{32, 48}).value_or(0) == 37 &&
                     free_map.first_free_in_bin(BinRange{48, 64}).value_or(0) == 50 &&
                     !free_map.first_free_in_bin(BinRange{0, 32}).has_value(),
                 cases);
    }

    {
        PrefixRouter router;
        router.insert(0b00010000ULL << 56, 3);
        router.insert(0b00010001ULL << 56, 7);
        router.insert(0b10000000ULL << 56, 11);
        const auto a = router.query(0b00010000ULL << 56);
        const auto b = router.query(0b00010001ULL << 56);
        const auto c = router.query(0b10000000ULL << 56);
        const auto missing = router.query(0b00010010ULL << 56);
        run_case("prefix_router_compressed_short_prefix",
                 a == 3 && b == 7 && c == 11 && !missing.has_value() &&
                     router.node_count() <= 5,
                 cases);
    }
    ...
    {
        const FacilityBucketMeta meta{7, 123, 0xfeedbeefULL};
        const auto encoded = encode_facility_bucket_meta(meta);
        const auto decoded = decode_facility_bucket_meta(encoded, 128);
        run_case("facility_bucket_meta_roundtrip",
                 decoded.has_value() && decoded->entry_count == meta.entry_count &&
                     decoded->encoded_bits == meta.encoded_bits &&
                     decoded->fingerprint == meta.fingerprint,
                 cases);
    }
